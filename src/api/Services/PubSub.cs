...
namespace Skystedt.Api.Services
{
    public interface IPubSub
    {
        ...
        Task Broadcast<T>(T data, string? currentConnectionId = null, CancellationToken cancellationToken = default);
        Task SendToUser<T>(T data, string userId, CancellationToken cancellationToken = default);
        Task DisconnetUser(string userId, CancellationToken cancellationToken = default);
    ...
    public class PubSub : IPubSub
    {
        ...
        private const string WebPubSubConnectionVariable = "WebPubSubConnectionString";
        ...
        {
            var connectionString = configuration[WebPubSubConnectionVariable]
                ?? throw new Exception($"Missing configuration: {WebPubSubConnectionVariable}");

            _client = new WebPubSubServiceClient(connectionString, HubName);

            _accessKey = Encoding.UTF8.GetBytes(ParseAccessKey(connectionString));
        ...
        public void ValidateCallback(HttpRequestData request)
        {
            ...
            if (connectionId != null && userId != null)
            {
                ...
                var hash = HMACSHA256.HashData(_accessKey, bytes);
        ...
        public async Task DisconnetUser(string userId, CancellationToken cancellationToken = default)
        {
        ...
        private static string ParseAccessKey(string connectionString)
        {
